...
var lean_125x125_sAdSrcId, lean_125x125_sAdSrcUrl;

lean_125x125_sAdSrcId = null;
lean_125x125_sAdSrcUrl = null;
 ...
 */
function lean_ad_125x125_hide_unused_fields($, poller) {
	"use strict";
	...
	$formFields.each(function() {
		...
		if($(this).hasClass('url')) {
			...
			$input.val('');
			$input.attr('placeholder', 'http://example.co');
			$input.siblings().hide();
...
} // end lean_ad_125x125_hide_unused_fields
...
function ad_125x125_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(lean_125x125_sAdSrcId).val(sImageUrl);
			jQuery(lean_125x125_sAdSrcUrl).val(jQuery(sHtml).attr('href'));
			
		// Otherwise, we'll just grab the image
		} else {
	
			sImageUrl = jQuery(sHtml).attr('src');		
			jQuery(lean_125x125_sAdSrcId).val(jQuery(sHtml).attr('src'));
			
		} // end if
		
		// Display the image in the admin area
		if(jQuery(lean_125x125_sAdSrcId).siblings('img').parent('a').length > 0) {
			jQuery(lean_125x125_sAdSrcId).siblings('img').parent('a').show();
		} // end if/else
		jQuery(lean_125x125_sAdSrcId).siblings('img').attr('src', sImageUrl).show();
		
		// Hide the upload anchor, show the delete anchor
		jQuery(lean_125x125_sAdSrcId).siblings('.ad_upload').hide();
		jQuery(lean_125x125_sAdSrcId).siblings('.ad_delete').show();
...
function lean_ad_125x125($) {
	...
	$('.lean-ad-125x125-wrapper img').unbind('click')
		.click(function() {	
	
		// Identify the input fields for this advertisement
		var sAdNumber =$(this).parent('span').attr('class').split(' ')[1];
		lean_125x125_sAdSrcId = $(this).siblings('.' + sAdNumber + '-src');
		lean_125x125_sAdSrcUrl = $(this).siblings('.' + sAdNumber + '-url');
		...
		$('#TB_iframeContent').load(function() {
			...
			var ad_125x125_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					lean_ad_125x125_hide_unused_fields($, ad_125x125_poll);
				} // end if
			}, 500);
	
			// if they aren't uplaoding, we'll clear the fields on load
			lean_ad_125x125_hide_unused_fields($);
			
		});
	
	});
	
	$('.lean-ad-125x125-wrapper a').click(function(evt) {
...
(function($) {
	...
	$(document).ajaxSuccess(function(e, xhr, settings) {
		...
		var sWidgetName = 'lean-ad-125x125';
