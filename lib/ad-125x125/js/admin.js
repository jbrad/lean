...
_standard_125x125_sAdSrcId = null;
_standard_125x125_sAdSrcUrl = null;
...
function standard_ad_125x125($) {
	...
	$('.standard-ad-125x125-wrapper img').click(function() {
		...
		_standard_125x125_sAdSrcId = $(this).siblings('.' + sAdNumber + '-src');
		_standard_125x125_sAdSrcUrl = $(this).siblings('.' + sAdNumber + '-url');
...
function ad_125x125_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(_standard_125x125_sAdSrcId).val(sImageUrl);
			jQuery(_standard_125x125_sAdSrcUrl).val(jQuery(sHtml).attr('href'));
			
		// Otherwise, we'll just grab the image
		} else {
	
			sImageUrl = jQuery(sHtml).attr('src');		
			jQuery(_standard_125x125_sAdSrcId).val(jQuery(sHtml).attr('src'));		
			
		} // end if
		
		// Display the image in the admin area
		if(jQuery(_standard_125x125_sAdSrcId).siblings('img').parent('a').length > 0) {
			jQuery(_standard_125x125_sAdSrcId).siblings('img').parent('a').show();
		} // end if/else
		jQuery(_standard_125x125_sAdSrcId).siblings('img').attr('src', sImageUrl).show();
		
		// Hide the upload anchor, show the delete anchor
		jQuery(_standard_125x125_sAdSrcId).siblings('.ad_upload').hide();
		jQuery(_standard_125x125_sAdSrcId).siblings('.ad_delete').show();
