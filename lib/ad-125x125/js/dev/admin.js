...
var ad_125x125_sAdSrcId, ad_125x125_sAdSrcUrl;

ad_125x125_sAdSrcId = null;
ad_125x125_sAdSrcUrl = null;
 ...
 */
function ad_125x125_hide_unused_fields($, poller) {
	"use strict";
...
} // end ad_125x125_hide_unused_fields
...
function ad_125x125_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(ad_125x125_sAdSrcId).val(sImageUrl);
			jQuery(ad_125x125_sAdSrcUrl).val(jQuery(sHtml).attr('href'));
			
		// Otherwise, we'll just grab the image
		} else {
	
			sImageUrl = jQuery(sHtml).attr('src');		
			jQuery(ad_125x125_sAdSrcId).val(jQuery(sHtml).attr('src'));
			
		} // end if
		
		// Display the image in the admin area
		if(jQuery(ad_125x125_sAdSrcId).siblings('img').parent('a').length > 0) {
			jQuery(ad_125x125_sAdSrcId).siblings('img').parent('a').show();
		} // end if/else
		jQuery(ad_125x125_sAdSrcId).siblings('img').attr('src', sImageUrl).show();
		
		// Hide the upload anchor, show the delete anchor
		jQuery(ad_125x125_sAdSrcId).siblings('.ad_upload').hide();
		jQuery(ad_125x125_sAdSrcId).siblings('.ad_delete').show();
...
function ad_125x125($) {
	...
	$('.ad-125x125-wrapper img').unbind('click')
		...
		ad_125x125_sAdSrcId = $(this).siblings('.' + sAdNumber + '-src');
		ad_125x125_sAdSrcUrl = $(this).siblings('.' + sAdNumber + '-url');
		...
		$('#TB_iframeContent').load(function() {
			...
			var ad_125x125_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					ad_125x125_hide_unused_fields($, ad_125x125_poll);
				} // end if
			}, 500);
	
			// if they aren't uplaoding, we'll clear the fields on load
			ad_125x125_hide_unused_fields($);
