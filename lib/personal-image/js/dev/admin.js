...
var personal_image_url, personal_image_src;
personal_image_url = null;
personal_image_src = null;
 ...
 */
function personal_image_hide_unused_fields($, poller) {
	"use strict";
...
} // end personal_image_hide_unused_fields
...
function personal_image($) {
	...
	*/
	...
	$('.personal-image-wrapper .option .personal_image_preview_image_container').unbind('click')
		.click(function(evt) {

		evt.preventDefault();

		// Identify the input fields for this image and anchor
		personal_image_url = '#' + $(this).siblings('.img_url').attr('id');
		personal_image_src = '#' + $(this).siblings('.img_src').attr('id');
		...
		$('#TB_iframeContent').load(function() {
			...
			var personal_image_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					personal_image_hide_unused_fields($, personal_image_poll);
				} // end if
			}, 500);

			// if they aren't uplaoding, we'll clear the fields on load
			personal_image_hide_unused_fields($);
		});
...
function personal_image_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length === 1) {
			...
			jQuery(personal_image_src).val(sImageUrl);
			jQuery(personal_image_url).val(jQuery(sHtml).attr('href'));

		} else {

			sImageUrl = jQuery(sHtml).attr('src');
			jQuery(personal_image_src).val(jQuery(sHtml).attr('src'));

		} // end if/else

		// Display the image in the admin area
		jQuery(personal_image_src).siblings('.personal_image_preview_image_container').children('img').attr('src', sImageUrl).show();

		// Hide the upload anchor, show the delete anchor
		jQuery(personal_image_src).siblings('.img_upload').hide();
		jQuery(personal_image_src).siblings('.img_delete').show();
