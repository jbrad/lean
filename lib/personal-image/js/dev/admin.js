...
var lean_personal_image_url, lean_personal_image_src;
lean_personal_image_url = null;
lean_personal_image_src = null;
 ...
 */
function lean_personal_image_hide_unused_fields($, poller) {
	"use strict";
...
} // end lean_personal_image_hide_unused_fields

/**
 * Attaches event handlers for lean's Personal Image widget administration
 * panel.
 */
function lean_personal_image($) {
	"use strict";
	...
	*/
	...
	$('.lean-personal-image-wrapper .option .personal_image_preview_image_container').unbind('click')
		.click(function(evt) {

		evt.preventDefault();

		// Identify the input fields for this image and anchor
		lean_personal_image_url = '#' + $(this).siblings('.img_url').attr('id');
		lean_personal_image_src = '#' + $(this).siblings('.img_src').attr('id');
		...
		$('#TB_iframeContent').load(function() {
			...
			var personal_image_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					lean_personal_image_hide_unused_fields($, personal_image_poll);
				} // end if
			}, 500);

			// if they aren't uplaoding, we'll clear the fields on load
			lean_personal_image_hide_unused_fields($);
		});

	});

	// Delete the adveritsement and clear the input fields when the delete anchor is clicked
	$('.lean-personal-image-wrapper .option .img_delete').click(function(evt) {
...
function personal_image_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length === 1) {
			...
			jQuery(lean_personal_image_src).val(sImageUrl);
			jQuery(lean_personal_image_url).val(jQuery(sHtml).attr('href'));

		} else {

			sImageUrl = jQuery(sHtml).attr('src');
			jQuery(lean_personal_image_src).val(jQuery(sHtml).attr('src'));

		} // end if/else

		// Display the image in the admin area
		jQuery(lean_personal_image_src).siblings('.personal_image_preview_image_container').children('img').attr('src', sImageUrl).show();

		// Hide the upload anchor, show the delete anchor
		jQuery(lean_personal_image_src).siblings('.img_upload').hide();
		jQuery(lean_personal_image_src).siblings('.img_delete').show();
...
(function($) {
	...
	$(document).ajaxSuccess(function(e, xhr, settings) {
		...
		var sWidgetName = 'lean-personal-image';
