...
_standard_personal_image_url = null;
...
function standard_personal_image($) {
	...
	$('.bio textarea').each(function() {
		...
		$span.text( 400 - parseInt($(this).val().length) );
	...
	*/
	...
	$('.standard-personal-image-wrapper .option .personal_image_preview_image_container').unbind('click')
		...
		_standard_personal_image_url = '#' + $(this).siblings('.img_url').attr('id');
...
function personal_image_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length === 1) {
			...
			jQuery(_standard_personal_image_src).val(sImageUrl);
			jQuery(_standard_personal_image_url).val(jQuery(sHtml).attr('href'));
		...
		jQuery(_standard_personal_image_src).siblings('.personal_image_preview_image_container').children('img').attr('src', sImageUrl).show();
				
		// Hide the upload anchor, show the delete anchor
		jQuery(_standard_personal_image_src).siblings('.img_upload').hide();
		jQuery(_standard_personal_image_src).siblings('.img_delete').show();
