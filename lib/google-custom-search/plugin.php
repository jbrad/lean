...
class Google_Custom_Search extends WP_Widget {
	...
	function __construct() {
		...
		$widget_opts = array(
			'classname' 	=> __( 'lean-google-custom-search', 'lean' ),
			'description' 	=> __( 'Easily add Google Custom Search to your Lean-powered blog.', 'lean' )
		);
		$this->WP_Widget( 'lean-google-custom-search', __( 'Google Custom Search', 'lean' ), $widget_opts );
	...
	function load_admin_stylesheets() {
		wp_enqueue_style( 'gcse-widget', get_template_directory_uri() . '/lib/google-custom-search/css/admin.css', false, LEAN_THEME_VERSION );
	...
	function load_stylesheets() {
		wp_enqueue_style( 'gcse-widget', get_template_directory_uri() . '/lib/google-custom-search/css/widget.css', false, LEAN_THEME_VERSION );
	} // end load_stylesheets
	...
	private function create_search_results_template() {
		...
		if( null == get_page_by_title( 'search results' ) ) {
			...
			// Get the content
			$page_content = wp_remote_get( get_template_directory_uri() . '/lib/google-custom-search/lib/Lean_Google_Custom_Search.template.php' );
			...
			$page_id = wp_insert_post(
				array(
					...
					'post_title'		=>	__( 'Search Results', 'lean' ),
					'post_status'		=>	'publish',
					'post_type'			=>	'page',
			...
			// Mark that this post was created by lean
	...
	public function existing_search_results_template() {
		...
		if( $this->is_page( 'widgets.php' ) ) {
			...
			if( 1 != get_post_meta( $page_id, 'lean_google_custom_search', true ) ) {
				...
				echo '<div id="lean-gcse-template-exists-notification" class="updated"><p>' . __( 'Could not configure Google Custom Search widget because the required "search-results" permalink is already in use. Please rename <a href="post.php?post=' . $page_id . '&action=edit">the existing page\'s permalink</a>, or <a href="post.php?post=' . $page_id . '&action=edit">delete the page</a>, and try again.', 'lean') . '</p></div>';
		...
		// Otherwise, if we're editing the generated Search Results page, then we need to display a message.
			...
			// Otherwise, they are on the post edit page so they should be able to see the message.
				...
				echo '<div id="lean-gcse-template-notification" class="updated"><p>' . __( 'This page was generated by Lean for use with the Google Custom Search widget. <a href="widgets.php">Remove the widget</a> from all sidebars to delete this page.', 'lean' ) . '</p></div>';
