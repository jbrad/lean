...
class Google_Custom_Search extends WP_Widget {
	...
	public function existing_search_results_template() {
		...
		// Otherwise, if we're editing the generated Search Resuklts page, then we need to display a message.
		} else if( $this->is_page( 'post.php' ) && 1 == get_post_meta( $page_id, 'standard_google_custom_search', true ) && isset( $_GET['action'] ) ) {
