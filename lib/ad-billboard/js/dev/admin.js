...
var lean_468x60_sAdSrcId, lean_468x60_sAdSrcUrl;

lean_468x60_sAdSrcId = null;
lean_468x60_sAdSrcUrl = null;
 ...
 */
function lean_ad_468x60_hide_unused_fields($, poller) {
	"use strict";
...
} // end lean_ad_468x60_hide_unused_fields
 ...
 */
function lean_ad_468x60_show_media_uploader() {
	"use strict";
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(lean_468x60_sAdSrcId).val(sImageUrl);
			jQuery(lean_468x60_sAdSrcUrl).val(jQuery(sHtml).attr('href'));

		// Otherwise, we'll just grab the image
		} else {

			sImageUrl = jQuery(sHtml).attr('src');
			jQuery(lean_468x60_sAdSrcId).val(jQuery(sHtml).attr('src'));

		} // end if

		// Display the image in the admin area
		jQuery(lean_468x60_sAdSrcId).parent()
		...
		jQuery(lean_468x60_sAdSrcId).siblings('.ad_delete')
			.show();
...
} // end lean_ad_468x60_show_media_uploader

/**
 * Attaches event handlers for lean's 468x60 advertisement widget administration
 * panel.
 */
function lean_ad_468x60($) {
	"use strict";
	...
	$('.lean-ad-468x60-wrapper > .preview_image_container').unbind('click')
		.click(function(evt) {
		...
		if( '' === sId ) {
			...
			lean_468x60_sAdSrcId = '#' + $('.ad_src').attr('id');
			lean_468x60_sAdSrcUrl = '#' + $('.ad_url').attr('id');

		// Otherwise, we're running in normal mode.
		} else {

			lean_468x60_sAdSrcId = '#' + $('#' + sId + ' .widget-content .lean-ad-468x60-wrapper').children('.ad_src').attr('id');
			lean_468x60_sAdSrcUrl = '#' + $('#' + sId + ' .widget-content .lean-ad-468x60-wrapper').children('.ad_url').attr('id');

		} // end if/else

		// Show the media uploader
		lean_ad_468x60_show_media_uploader();
		$('#TB_iframeContent').load(function() {
			...
			var ad_468x60_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					lean_ad_468x60_hide_unused_fields($, ad_468x60_poll);
				} // end if
			}, 500);

			// if they aren't uploading, we'll clear the fields on load
			lean_ad_468x60_hide_unused_fields($);
	...
	$('.lean-ad-468x60-wrapper > .ad_delete').click(function(evt) {
...
(function($) {
	...
	$(document).ajaxSuccess(function(e, xhr, settings) {
		...
		var sWidgetName = 'lean-ad-468x60';
