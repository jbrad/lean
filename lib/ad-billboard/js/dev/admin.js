...
var ad_468x60_sAdSrcId, ad_468x60_sAdSrcUrl;

ad_468x60_sAdSrcId = null;
ad_468x60_sAdSrcUrl = null;
 ...
 */
function ad_468x60_hide_unused_fields($, poller) {
	"use strict";
...
} // end ad_468x60_hide_unused_fields
 ...
 */
function ad_468x60_show_media_uploader() {
	"use strict";
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(ad_468x60_sAdSrcId).val(sImageUrl);
			jQuery(ad_468x60_sAdSrcUrl).val(jQuery(sHtml).attr('href'));

		// Otherwise, we'll just grab the image
		} else {

			sImageUrl = jQuery(sHtml).attr('src');
			jQuery(ad_468x60_sAdSrcId).val(jQuery(sHtml).attr('src'));

		} // end if

		// Display the image in the admin area
		jQuery(ad_468x60_sAdSrcId).parent()
		...
		jQuery(ad_468x60_sAdSrcId).siblings('.ad_delete')
			.show();
...
} // end ad_468x60_show_media_uploader
...
function ad_468x60($) {
	...
	$('.ad-468x60-wrapper > .preview_image_container').unbind('click')
		.click(function(evt) {
		...
		if( '' === sId ) {
			...
			ad_468x60_sAdSrcId = '#' + $('.ad_src').attr('id');
			ad_468x60_sAdSrcUrl = '#' + $('.ad_url').attr('id');

		// Otherwise, we're running in normal mode.
		} else {

			ad_468x60_sAdSrcId = '#' + $('#' + sId + ' .widget-content .ad-468x60-wrapper').children('.ad_src').attr('id');
			ad_468x60_sAdSrcUrl = '#' + $('#' + sId + ' .widget-content .ad-468x60-wrapper').children('.ad_url').attr('id');

		} // end if/else

		// Show the media uploader
		ad_468x60_show_media_uploader();
		$('#TB_iframeContent').load(function() {
			...
			var ad_468x60_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					ad_468x60_hide_unused_fields($, ad_468x60_poll);
				} // end if
			}, 500);

			// if they aren't uploading, we'll clear the fields on load
			ad_468x60_hide_unused_fields($);
	...
	$('.ad-468x60-wrapper > .ad_delete').click(function(evt) {
