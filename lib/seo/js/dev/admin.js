...
function cleanTitle($, words) {
	...
	var cleanWords, characters, illegalChars, index;
	cleanWords = [];
	characters = [ '~', '!', '@', '#', '$', '^', '&', '*', '(', ')', '-', '=', '+', '[', '{', ']', '}', ':', '\'', '"', ',', '.', '?' ];
	illegalChars = [ '%', '\\', '|', ';', '/', '<', '>', '_' ];
	...
	if ( 0 < words.length ) {
		...
		$(words).each(function() {
			...
			// If this is an illegal character, do nothing
		...
		// Now if any of the characters in the cleanWords are illegal words, remove them
		$(words).each(function() {

			if( -1 < $.inArray( this, illegalChars ) ) {
				cleanWords.splice( cleanWords.indexOf( this ), 1 );
			} // end if
