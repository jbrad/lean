 ...
 *
 * @params	$			A reference to the jQuery function
 * @params	sWord		A reference to the word to strip of special characters
 * @return  sCleanWord	The cleaned word
 */
function stripSpecialCharacters( $, sWord ) {
	"use strict";

	return $.trim( sWord ).replace(/[^\w\s]/gi, '' );
...
function updateMetaDescription($, $this) {
	...
	var oMatch, aTitleWords, aMatches, sCurrentWord, oRegEx, i, l, sCurrentMatch;
	...
	if(0 < aTitleWords.length && 0 < $.trim(aTitleWords[0]).length ) {
		...
		for( i = 0, l = aTitleWords.length; i < l; i++) {
			...
			sCurrentWord = stripSpecialCharacters( $, encodeURIComponent( aTitleWords[i] ) );
			...
			if( '' !== sCurrentWord ) {
				...
				if( null !== ( oMatch = $('#standard_seo_post_meta_description').val().match( oRegEx ) ) ) {
	...
	$(aMatches).each(function() {
		...
		for( i = 0, l = this.length; i < l; i++) {
			...
			if( '' !== sCurrentWord ) {
				...
				oRegEx = new RegExp( '\\b' + sCurrentWord + '\\b', 'g' );

				// Finally, update the HTML of the description
				$('#description').html(
					$('#description').html().replace(oRegEx, '<strong>' + sCurrentWord + '</strong>')
				);
