	...
	if(0 < aTitleWords.length && 0 < $.trim(aTitleWords[0]).length ) {
		...
		for( i = 0, l = aTitleWords.length; i < l; i++) {
			...
			if( '' !== sCurrentWord ) {
				...
				oMatch = $('#lean_seo_post_meta_description').val().match( oRegEx );
				if( null !== oMatch ) {
...
(function($) {
	...
	$(function() {
		...
		if( 0 < $('#post-title').length ) {
			...
			updateMetaDescription($, $('#lean_seo_post_meta_description'));
			$('#lean_seo_post_meta_description').keyup(function() {
