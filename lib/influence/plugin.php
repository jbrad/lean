...
class Social_Influence extends WP_Widget {
	...
	private function twitter_follower_count( $username, $debug = false ) {
		...
		// If the value doesn't exist as a transient, let's read the value from Twitter
			...
			$response = $this->get_feed_response( 'https://twitter.com/' . $username . '/' );
	...
	private function get_total_influence_count( $twitter, $facebook, $debug = false ) {
		...
		if( '' != $twitter ) {
			$influence += str_ireplace( ',', '', $this->twitter_follower_count( $twitter, $debug ) );
		} // end if

		if( '' != $facebook ) {
			$influence += str_ireplace( ',', '', $this->facebook_like_count( $facebook, $debug ) );
		} // end if
