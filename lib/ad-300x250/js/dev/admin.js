...
var ad_300x250_sAdSrcId, ad_300x250_sAdSrcUrl;

ad_300x250_sAdSrcId = null;
ad_300x250_sAdSrcUrl = null;
 ...
 */
function ad_300x250_hide_unused_fields($, poller) {
	"use strict";
...
} // end ad_300x250_hide_unused_fields
 ...
 */
function ad_300x250_show_media_uploader() {
	"use strict";
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(ad_300x250_sAdSrcId).val(sImageUrl);
			jQuery(ad_300x250_sAdSrcUrl).val(jQuery(sHtml).attr('href'));

		// Otherwise, we'll just grab the image
		} else {

			sImageUrl = jQuery(sHtml).attr('src');
			jQuery(ad_300x250_sAdSrcId).val(jQuery(sHtml).attr('src'));

		} // end if

		// Display the image in the admin area
		jQuery(ad_300x250_sAdSrcId).parent()
		...
		jQuery(ad_300x250_sAdSrcId).siblings('.ad_delete')
			.show();
...
} // end ad_300x250_show_media_uploader
...
function ad_300x250($) {
	...
	$('.ad-300x250-wrapper > .preview_image_container').unbind('click')
		...
		if( '' === sId ) {
			...
			ad_300x250_sAdSrcId = '#' + $('.ad_src').attr('id');
			ad_300x250_sAdSrcUrl = '#' + $('.ad_url').attr('id');

		// Otherwise, we're running in normal mode.
		} else {

			ad_300x250_sAdSrcId = '#' + $('#' + sId + ' .widget-content .ad-300x250-wrapper').children('.ad_src').attr('id');
			ad_300x250_sAdSrcUrl = '#' + $('#' + sId + ' .widget-content .ad-300x250-wrapper').children('.ad_url').attr('id');

		} // end if/else

		// Show the media uploader
		ad_300x250_show_media_uploader();
		$('#TB_iframeContent').load(function() {
			...
			var ad_300x250_poll = setInterval(function() {
				if($('#TB_iframeContent').contents().find('#media-items').children().length > 0) {
					ad_300x250_hide_unused_fields($, ad_300x250_poll);
				} // end if
			}, 500);

			// if they aren't uplaoding, we'll clear the fields on load
			ad_300x250_hide_unused_fields($);
