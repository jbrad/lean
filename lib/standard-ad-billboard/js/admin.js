...
_standard_468x60_sAdSrcId = null;
_standard_468x60_sAdSrcUrl = null;
...
function standard_ad_468x60($) {
	...
	$('.standard-ad-468x60-wrapper > .preview_image_container').unbind('click')
		...
		if( '' === sId ) {
			...
			_standard_468x60_sAdSrcId = '#' + $('.ad_src').attr('id');
			_standard_468x60_sAdSrcUrl = '#' + $('.ad_url').attr('id');
		
		// Otherwise, we're running in normal mode.
		} else {
		
			_standard_468x60_sAdSrcId = '#' + $('#' + sId + ' .widget-content .standard-ad-468x60-wrapper').children('.ad_src').attr('id');
			_standard_468x60_sAdSrcUrl = '#' + $('#' + sId + ' .widget-content .standard-ad-468x60-wrapper').children('.ad_url').attr('id');
...
function standard_ad_468x60_show_media_uploader() {
	...
	window.send_to_editor = function(sHtml) {
		...
		if(jQuery(sHtml).children('img').length > 0) {
			...
			jQuery(_standard_468x60_sAdSrcId).val(sImageUrl);
			jQuery(_standard_468x60_sAdSrcUrl).val(jQuery(sHtml).attr('href'));
			
		// Otherwise, we'll just grab the image
		} else {
	
			sImageUrl = jQuery(sHtml).attr('src');		
			jQuery(_standard_468x60_sAdSrcId).val(jQuery(sHtml).attr('src'));		
			
		} // end if
		
		// Display the image in the admin area
		jQuery(_standard_468x60_sAdSrcId).parent()
		...
		jQuery(_standard_468x60_sAdSrcId).siblings('.ad_delete')
			.show();
